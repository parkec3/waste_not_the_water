...
def data_import_clean():
    ...
    # Rerrange rows based on capacity in ascending order
    dff = dff.sort_values('Capacity')
    return dff
...
def create_basic_list(dff):
    ...
    for i in capacity:
        # Scale the capacity down for markersize on map
        magnitudes.append(i/100000)
...
def get_color_graph(lons, lats, capacity):
    ...
    base_map.drawmeridians(np.arange(0, 360, 5), labels=[
                           False, True, True, False])
    base_map.drawparallels(np.arange(-90, 90, 5),
...
def get_size_map(lons, lats, magnitudes):
    ...
    base_map1.drawmeridians(np.arange(0, 360, 5), labels=[
                            False, True, True, False])
    base_map1.drawparallels(np.arange(-90, 90, 5),
                            labels=[False, True, True, False])

    min_marker_size = 7.5
    for lon, lat, mag in zip(lons, lats, magnitudes):
        x, y = base_map1(lon, lat)
        # Here makes sure that the dots size are based on their capacity.
...
def create_interactive_map(lat, lon, capacity):
    ...
    for lat0, lon0, cap in zip(lat, lon, capacity):
        folium.Marker([lat0, lon0], popup="Capacity: "+str(cap)+" Latitude:" +
                      str(lat0)+",Longitude: "+str(lon0)).add_to(interactive_map)
...
def interactive_customer_map(customer, i):
    ...
    interactive_map = folium.Map(
        location=[25, 45], zoom_start=3)  # Setup the basic interactive model
    # Make a single-row DataFrame of interested row of customer DataFrame
    customer_new = customer.loc[[i]]
    # Get a result DataFrame from Machine Learning Model
    result = waste_not_the_water.nearest_n.NP_removal(customer_new)
    # Make lists from result to be ready for add points to map
    lat_result = result['Latitude'].tolist()

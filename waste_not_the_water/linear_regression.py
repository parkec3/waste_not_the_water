...
def data_cleaning():
    ...
    df_cleaned = pd.DataFrame(data = {'LoadEntering': df_cap2['uwwLoadEnteringUWWTP'],'Capacity': df_cap2['uwwCapacity'], 
                            'NRemoval':df_cap2['uwwNRemoval'],'PRemoval':df_cap2['uwwPRemoval'],
                            'Longitude': df_cap2['uwwLongitude'], 'Latitude':df_cap2['uwwLatitude']})
...
def linear_regression_result():
    ...
    filename_lr, y_predict_lr = linear_regr(train, test[['LoadEntering','Longitude','Latitude']])
...
def split_train_test(df):
    ...
    train,test = train_test_split(df_split, test_size = 0.1, random_state=1010)
    return train,test
...
def ridge_regr(train, test):
    ...
    heat_ridge.fit(train_normalized[['Latitude','LoadEntering','Longitude']],train_normalized.Capacity)
    y_predict = heat_ridge.predict(test_normalized[['Latitude','LoadEntering','Longitude']])
...
def customer_inter(customer, filename_lr, filename_rr):
    ...
    y_customer_lr = result.predict(customer[['Latitude', 'Longitude', 'LoadEntering']])
    y_customer_rr = heat_ridge.predict(customer[['Latitude', 'LoadEntering', 'Longitude']])
